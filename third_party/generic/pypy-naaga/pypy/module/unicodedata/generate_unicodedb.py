...
def read_unicodedata(unicodedata_file, exclusions_file, east_asian_width_file,
                     unihan_file=None, case_fold=None):
    rangeFirst = {}
    rangeLast = {}
    ...
    for line in case_fold:
        ...
        if status.strip() in ['C', 'F']:
            l = [int(c, 16) for c in two.split(' ') if c]
            table[code].casefold = l
...
'''
    ...
    for code in range(len(table)):
        ...
        if table[code].casefold:
            casefold[code] = table[code].casefold
    ...
    writeDict(outfile, '_casefold', casefold, base_mod)
    print >> outfile, '''
def toupper(code):
    try:
...
def casefold(code):
    try:
        ...
        if base_mod is not None and code not in _totitle_corrected:
            return base_mod._casefold.get(code, [code])
        else:
            return [code]
'''
    # Decomposition
...
def main():
    ...
    casefold = open('CaseFolding-%s.txt' % options.unidata_version)
