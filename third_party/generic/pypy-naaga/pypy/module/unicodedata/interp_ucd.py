...
class UCD(Wrappable):
    def __init__(self, unicodedb):
        ...
        self._casefold = unicodedb._casefold
    ...
    def casefold(self, space, w_unichr):
        code = unichr_to_code_w(space, w_unichr)
        return space.newlist([space.wrap(c) for c in self._casefold(code)])
    casefold.unwrap_spec = ['self', ObjSpace, W_Root]
